...
class Package_control extends CI_Controller {
...
// fungsi index dijalankan pada saat pertama kali controller di panggil
	public function index()
	{
		if($this->client_logon){
			if($this->client_access=='0'){	redirect('logout');	}
			$this->view_package_all($this->client_logon);
		}else{
			redirect('login');
		}
	}
...
// fungsi view_package untuk menampilkan data paket yang dipilih oleh pelanggan
	public function view_package_all($custcode)
	{
		...
		$packages = $this->Package_model->get_package_homeview($customer->CUSTOMERID)->result();
			...
			// echo anchor_popup('friends/index/'.$uid.'/'.$suid,'add as friend',$attr);
				if($package->IDPAKET=='99983' || $package->IDPAKET=='99984' || $package->IDPAKET=='99999'){
					$this->table->add_row(++$i,$package->PACKAGE,
							$package->DESCRIPTION, 
							$package->MAXCHANEL,
							anchor('package_control/view_alacarte/'.$package->pktid,'Detil',array('class' => 'btn package')));
				}else{		
					$this->table->add_row(++$i,$package->PACKAGE,
							$package->DESCRIPTION, 
							$package->MAXCHANEL,
							anchor('package_control/view_packagedet/'.$package->pktid,'Detil',array('class' => 'btn package')));
				}
			}
...
// end of function view_package

// fungsi untuk menampilkan data ala carte
	public function view_alacarte($trpackageid){
...
// fungsi untuk menampilkan data paket secara detail apa saja yang dipilih nya
	public function view_packagedet($trpackageid)
	{
			...
			if ($valedit == TRUE){
				...
				$data['editlink'] = $base_url."package_control/edit_paket/".$trpackageid;
			}
	...
	//		<img class="selected" src="images/actived.png">
			...
			if ($valedit== TRUE){
				...
				$data['savelink'] = $base_url."package_control/update_paket/".$trpackageid;
				$data['packagelink'] = $base_url."package_control/view_packagedet/".$trpackageid;
			}
...
function change_result($trpackageid,$savestatus){
		if($savestatus=='0'){$data['err_msg'] = '<span class="notif-msg" id="succeed">Perubahan channel sedang kami proses, Anda dapat menikmati dalam waktu 5 menit</span>';}
		else{$data['err_msg'] = '<span class="notif-msg" id="error">Anda belum melakukan perubahan. Mohon lakukan modifikasi pada pilihan channel premium Anda</span>';}
		$result = $this->Package_model->get_service_id()->result();
		$chtabs = $this->Package_model->get_channel_category()->result();
		$chusers = $this->Package_model->get_ch_used($trpackageid)->result();
		$ch = 0;
...
/* End of file Package_control.php */
/* Location: ./system/application/controllers/Package_control.php */