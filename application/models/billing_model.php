...
// fungsi untuk mendapatkan billing summary per 6 bulan terakhir	
	public function get_bill_summary($custcode)
	{
...
*/
		...
		$this->baca->select('t0.HISTORYID');
		$this->baca->select('t0.CUSTOMERID');
		$this->baca->select('t0.CUSTOMERCODE');
		$this->baca->select('t0.BILLINGTYPE');
		$this->baca->select('t0.INVOICENO');
		$this->baca->select("date_format(t0.TRANSACTIONDATE,'%d-%b-%Y') as TRANSACTIONDATE", FALSE);
		$this->baca->select("date_format(t0.RECEIPTDATE,'%d-%b-%Y') as RECEIPTDATE", FALSE);
		$this->baca->select("format(round(t0.AMOUNT),0) as AMOUNT",FALSE);
		$this->baca->select('t0.BALANCE');
		
		$this->baca->from('T_BILLING_HIST as t0');		
		
		$this->baca->where('t0.CUSTOMERCODE',$custcode);
		$wheredate = "date_format(t0.TRANSACTIONDATE,'%Y-%m-%d')>(curdate() - interval 6 month)";
		$this->baca->where($wheredate);
...
// fungsi untuk menampilkan billing header
	public function get_billing_h($bilhistid)
	{
		$this->baca->initialize();
/*		
		$this->baca->select("t1.ATRBTID as HISTORYID");
		...
		$this->baca->select("t5.PHONE");
		$this->baca->select("t5.MOBILE");
		$this->baca->select("t5.EMAIL");
...
//*/	
		...
		$this->baca->select('HISTORYID');
		$this->baca->select('CUSTOMERID');
		$this->baca->select('CUSTOMERCODE');
		$this->baca->select('CUSTOMERNAME');
		$this->baca->select('CONTRACTNO');
		$this->baca->select('INVOICENO');
		$this->baca->select('INVno');
		$this->baca->select("date_format(TRANSACTIONDATE,'%d-%b-%Y') AS TRANSACTIONDATE",FALSE);
		$this->baca->select("date_format(RECEIPTDATE,'%d-%b-%Y') AS RECEIPTDATE",FALSE);
		$this->baca->select('format(round(AMOUNT),0) as AMOUNT',FALSE);
		$this->baca->select('BILLINGADDRESS');
		$this->baca->select('PHONE');
		$this->baca->select('MOBILE');
		$this->baca->select('FAX');
		$this->baca->select('CUSTEMAIL');
		$this->baca->select('DEFAULTPWD');
		$this->baca->select('EMAIL');
		$this->baca->select('NPWP');
		$this->baca->select('TRANSACTIONDATE AS BLNTHN');

		$this->baca->from('T_BILLING_HEAD');
		
		$this->baca->where("HISTORYID",  $bilhistid);
...
// fungsi untuk mendapatkan jumlah administrasi dan ppn	
	public function get_adm($custcode,$bln,$thn)
	{	
		$this->baca->initialize();
...
// fungsi untuk mendapatkan billing detail untuk perhitungan
	public function get_billing_d($historyid)
	{
		$this->baca->initialize();
/*		
		...
		//$this->db->order_by("t1.ATRBICID","DESC");
*/

		$this->baca->select("ATRBTID");
		$this->baca->select("INVOICENO");
		$this->baca->select("ATRBAIDP");
		$this->baca->select("ATRBTIDP");
		$this->baca->select("date_format(DUEDATE,'%d-%b-%Y') AS DUEDATE",FALSE);
		$this->baca->select("date_format(INVOICE_PERIODE,'%d-%b-%Y') AS INVOICE_PERIODE",FALSE);
		$this->baca->select("TRBCD");
		$this->baca->select("DESCRIPTION");
		$this->baca->select("format(QUANTITY,2) AS QUANTITY",FALSE);
		$this->baca->select("PRICE");
		$this->baca->select("TOTAL_PRICE AS TOTAL_PRICE");
		$this->baca->select("PERIODE");
		$this->baca->from("T_BILLING_DET");
		$this->baca->where("ATRBTIDP",$historyid);
		
		return $this->baca->get();
		
		$this->baca->close();
	}
// end of get billing det

// billing footer untuk perhitungan total yang harus dibayar oleh pelanggan	
	public function get_billing_footer($invoiceno,$custcode)
	{
		$this->baca->initialize();
...
// end of billing footer

// fungsi untuk mendapatkan penyesuaian
	public function get_bill_adjust($custcode,$invoice)
	{
		$this->baca->initialize();
		...
		$this->baca->select("date_format(t1.ATRBTDTD,'%d-%b-%Y') AS RECEIPTDATE", FALSE);
		//$this->baca->select("format((sum(t2.ATRBTVAL) * if(find_in_set(2,t3.ATRBTTYFLAG),1,-(1))),0) AS AMOUNT", FALSE);
		$this->baca->select("(sum(t2.ATRBTVAL) * if(find_in_set(2,t3.ATRBTTYFLAG),1,-(1))) AS AMOUNT", FALSE);
		$this->baca->select("concat('l1+u1') AS BALANCE");
		...
		$wherein = 't3.ATRBTCID in (3,4,6)';
		$this->baca->where($wherein);
		//$wherenote =array('Credit Note','Debit Note');
		$wherenote =("t3.ATRBTTYNM IN ('Credit Note','Debit Note')");
		$this->baca->where($wherenote);
//		$wheredate = "date_format(t1.ATRBTDT,'%Y-%m-%d') > (curdate() - interval 6 month)";
//		$this->baca->where($wheredate);
		$this->baca->where("t0.ATRBAP03",  $custcode);
		$this->baca->where("t1.ATRBTNOG",  $invoice);
