...
class Package extends CI_Controller {
	...
	function view_package_all($custcode)
	{
		...
		$this->table->set_heading('No', 'Package Name','Description','Active');
		...
		foreach ($packages as $package)
			...
			// echo anchor_popup('friends/index/'.$uid.'/'.$suid,'add as friend',$attr);
				if($package->IDPAKET=='99983' || $package->IDPAKET=='99984' || $package->IDPAKET=='99999'){
					...
					anchor('package/view_alacarte/'.$package->pktid,$package->PACKAGE,array('class' => 'package')),
							$package->DESCRIPTION, 
							$package->MAXCHANEL);
				}else{		
					$this->table->add_row(++$i,
					anchor('package/view_packagedet/'.$package->pktid,$package->PACKAGE,array('class' => 'package')),
							$package->DESCRIPTION, 
							$package->MAXCHANEL);
				}
			}
...
function change_result($trpackageid,$savestatus){
		if($savestatus=='0'){$data['err_msg'] = 'Your request has been sent, please wait about 5 minutes until channel updated';}
		else{$data['err_msg'] = "Please select channel to edit";}
		$result = $this->Package_model->get_service_id()->result();
		$chtabs = $this->Package_model->get_channel_category()->result();
		$chusers = $this->Package_model->get_ch_used($trpackageid)->result();
		$ch = 0;
